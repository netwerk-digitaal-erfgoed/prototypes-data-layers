...
import { Document, search, SearchResult, retrieve } from "@repo/typesense/persons";
...
function getNavPages(baseUri: string, queryString: string, searchResult: SearchResult) {
  ...
  navPages.set("current", `${baseUri}/persons/page/${currentPage}${queryString}`);

  if (searchResult.found > 0) {
    navPages.set("first", `${baseUri}/persons/page/1${queryString}`);
    navPages.set("last", `${baseUri}/persons/page/${lastPage}${queryString}`);
  }

  if (nextPage <= lastPage) {
    navPages.set("next", `${baseUri}/persons/page/${nextPage}${queryString}`);
  }

  if (prevPage > 0) {
    navPages.set("prev", `${baseUri}/persons/page/${prevPage}${queryString}`);
  }
...
app.get("/v1/persons", sValidator("query", collectionQuerySchema), async (c) => {
...
function buildCollectionResponse(
  ...
  const response = {
    id: `${baseUri}/persons${queryString}`,
...
function buildDocumentResponse(baseUri: string, document: Document) {
  const response = {
    id: `${baseUri}/persons/${document.id}`,
    type: document.type,
    name: document.name,
  };
...
app.get("/v1/persons/:id", sValidator("param", singleResourceParamsSchema), async (c) => {
  const params = c.req.valid("param");
