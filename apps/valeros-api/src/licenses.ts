...
import { Document, search, SearchResult, retrieve } from "@repo/typesense/licenses";
...
function getNavPages(baseUri: string, queryString: string, searchResult: SearchResult) {
  ...
  navPages.set("current", `${baseUri}/licenses/page/${currentPage}${queryString}`);

  if (searchResult.found > 0) {
    navPages.set("first", `${baseUri}/licenses/page/1${queryString}`);
    navPages.set("last", `${baseUri}/licenses/page/${lastPage}${queryString}`);
  }

  if (nextPage <= lastPage) {
    navPages.set("next", `${baseUri}/licenses/page/${nextPage}${queryString}`);
  }

  if (prevPage > 0) {
    navPages.set("prev", `${baseUri}/licenses/page/${prevPage}${queryString}`);
  }
...
app.get("/v1/licenses", sValidator("query", collectionQuerySchema), async (c) => {
...
function buildCollectionResponse(
  ...
  const response = {
    id: `${baseUri}/licenses${queryString}`,
...
function buildDocumentResponse(baseUri: string, document: Document) {
  const response = {
    id: `${baseUri}/licenses/${document.id}`,
    type: document.type,
    name: document.name,
...
app.get("/v1/licenses/:id", sValidator("param", singleResourceParamsSchema), async (c) => {
  const params = c.req.valid("param");
