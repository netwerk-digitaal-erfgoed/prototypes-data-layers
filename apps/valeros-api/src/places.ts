...
import { Document, search, SearchResult, retrieve } from "@repo/typesense/places";
...
function getNavPages(baseUri: string, queryString: string, searchResult: SearchResult) {
  ...
  navPages.set("current", `${baseUri}/places/page/${currentPage}${queryString}`);

  if (searchResult.found > 0) {
    navPages.set("first", `${baseUri}/places/page/1${queryString}`);
    navPages.set("last", `${baseUri}/places/page/${lastPage}${queryString}`);
  }

  if (nextPage <= lastPage) {
    navPages.set("next", `${baseUri}/places/page/${nextPage}${queryString}`);
  }

  if (prevPage > 0) {
    navPages.set("prev", `${baseUri}/places/page/${prevPage}${queryString}`);
  }
...
app.get("/v1/places", sValidator("query", collectionQuerySchema), async (c) => {
...
app.get("/v1/places/:id", sValidator("param", singleResourceParamsSchema), async (c) => {
  const params = c.req.valid("param");
