...
import { Document, search, SearchResult, retrieve } from "@repo/typesense/organizations";
...
function getNavPages(baseUri: string, queryString: string, searchResult: SearchResult) {
  ...
  navPages.set("current", `${baseUri}/organizations/page/${currentPage}${queryString}`);

  if (searchResult.found > 0) {
    navPages.set("first", `${baseUri}/organizations/page/1${queryString}`);
    navPages.set("last", `${baseUri}/organizations/page/${lastPage}${queryString}`);
  }

  if (nextPage <= lastPage) {
    navPages.set("next", `${baseUri}/organizations/page/${nextPage}${queryString}`);
  }

  if (prevPage > 0) {
    navPages.set("prev", `${baseUri}/organizations/page/${prevPage}${queryString}`);
  }
...
app.get("/v1/organizations", sValidator("query", collectionQuerySchema), async (c) => {
...
function buildCollectionResponse(
  ...
  const response = {
    id: `${baseUri}/organizations${queryString}`,
...
function buildDocumentResponse(baseUri: string, document: Document) {
  const response = {
    id: `${baseUri}/organizations/${document.id}`,
    type: document.type,
    name: document.name,
  };
...
app.get("/v1/organizations/:id", sValidator("param", singleResourceParamsSchema), async (c) => {
  const params = c.req.valid("param");
